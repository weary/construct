...
def fix_caller_afterwards(func):
    """ decorator for if the user might have different authorisation after the call"""
...
class Avatar(object):
    def __init__(self, core, nick, description):
        ...
        self.uid = self.core.shandler.our_serverid + "AAAAAA"
        ...
        for funcname, func in list(self.__class__.__dict__.items()):
    ...
    def test_access(self, cmd, args, user, forhelp):
        ...
        channel object.
        if forhelp is true args should be None
        """
        print("XXXXX test_access %r %s %s" %
              (cmd.funcname, cmd.minauth, user.level()))
        print("XXXXXX %r" % (args,))

        assert forhelp and args is None or isinstance(args, dict)

        if args and 'chan' in args:
            chan = self.core.channels.get_channel(args['chan'])
            if not chan:
                print("XXXXXX channel %s does not exist" % args["chan"])
                raise IrcMsgException(
                    user, "unknown channel '%s'" % args['chan'])
            args['chan'] = chan

        if cmd.minauth is guestlevel:
            print("XXXXXX ok - guest command")
            return  # everyone is at least guest

        userlevel = user.level()
        if userlevel is guestlevel:
            print("XXXXXX not allowed, user is guest")
            raise IrcMsgException(
                user, "guests access not allowed, please identify/register")
        assert (
            userlevel is registeredlevel
            or userlevel is confirmedlevel
            or userlevel is operlevel)
        ...
        if cmd.minauth is self.commands.chanoper:
            if not forhelp:
                ...
                if not chan:
                    # not reachable
                    print("XXXXXX not allowed, no channel found")
                    raise Exception("no channel found in arguments")
                if not chan.registered:
                    print("XXXXXX not allowed, channel not registered")
                    raise IrcMsgException(user,
                                          "'%s' is not a registered channel" %
                                          chan.name)
                if not chan.find_role(user) is operrole:
                    print(
                        "XXXXXX not allowed, user is not channel operator on %s"
                        % chan.name)
                    raise IrcMsgException(
                        user, "'%s' is not a channel operator on '%s'" %
                        (user.nick, chan.name))
            else:  # for help
                assert args is None
                # when serving help pages, we list the channel commands if the user is
                # channel operator on any channel
                if userlevel is not operlevel and not any(
                        chan.registered and chan.find_role(user) is operrole
                        for chan in self.core.channels.get_channels_with_user(user)):
                    raise IrcMsgException(
                        user, "user is not channel operator on any channel")
            print("XXXXXXX ok, command allowed")
            return  # ok

        if userlevel is operlevel:
            print("XXXXXXX ok, serveroperator")
            return  # serverops get all

        if cmd.minauth is confirmedlevel and userlevel is registeredlevel:
            print("XXXXXXXXX not ok, profile not confirmed")
            raise IrcMsgException(user, "profile not confirmed")

        if cmd.minauth is operlevel and userlevel is not operlevel:
            print("XXXXXXXXX not ok, not server operator")
            raise IrcMsgException(user, "not server operator")

        # it's OK
        print("XXXXXXX ok, end-of-function")

    def introduce(self):
        shandler = self.core.shandler
        now = int(time.time())
        servername = shandler.name
        username = self.nick  # username must be something valid
        msg = ":%s UID %s 1 %d +i %s %s 0 %s * :%s" % (
            shandler.our_serverid,
            self.nick,
            now,
            username,
            servername,
    ...
    def find_a_profile_for_nick(self, asker, nick):
        user = self.core.users.get_user_by_nick_yes_really(nick)
        profile = self.core.profiles.find_profile_by_nickname(nick)
        if user and profile:
            if user.profile is profile:
                return profile  # identified user
            raise IrcMsgException(
                asker, "Conflicting profile and logged-in user found for '%s'" % nick)
        if profile:
            return profile
    ...
    def rolesline(self, profile):
        ...
        if roles:
            roles = make_list((role_as_text(role) + " in " +
                               make_list(sort(channels),
                                         "and") for role,
                               channels in list(roles.items())),
                              "and")
        else:
    ...
    def cmd_identify(self, caller, cmd, nick, password):
        ...
        if caller.profile:
            raise IrcMsgException(
                caller, "already identified as %s" % caller.profile.register_nick)

        nick = nick or caller.nick
        profile = self.core.profiles.find_profile_by_nickname(nick)
        if not profile:
            raise IrcMsgException(caller,
                                  "No profile for %s, please register first" %
                                  nick)
        ...
        if user_ and user_ is not caller:
    ...
    def cmd_register(self, caller, cmd, password):
        ...
        if self.core.channels.get_channel(password):
            raise IrcMsgException(
                caller,
                "Trying to register a channel? try the 'register channel' command")

        profile = self.core.profiles.find_profile_by_nickname(caller.nick)
        if profile:
            raise IrcMsgException(
                caller, "User %s already registered" % caller.nick)
        ...
        if not password.isascii():
            raise IrcMsgException(
                caller, "Password may only contain ascii characters")
        newprofile = self.core.profiles.create_profile(caller.nick, password)
        self.notice(
            caller,
            (
                "Successfully registered %s, please remember your "
                + "password to identify next time")
            % caller.nick,)
    ...
    def cmd_unregister(self, caller, cmd, nick, password):
        ...
        if caller.level() is operlevel:
            ...
            if password:
                raise IrcMsgException(
                    caller, "Server operators should only specify a nickname")
            if not nick:
                raise IrcMsgException(
                    caller, "Server operators must specify a nickname")
            profile = self.find_a_profile_for_nick(caller, nick)
            if profile is caller.profile:
                raise IrcMsgException(
                    caller,
                    "Server operators cannot unregister their own profile")
        else:
            if nick and password:
                profile = self.find_a_profile_for_nick(caller, nick)
                if profile is not caller.profile:
                    raise IrcMsgException(
                        caller, "You can only unregister your own profile")
            else:
    ...
    def cmd_passwd(self, caller, cmd, oldpass, newpass):
        ...
        if not profile:
            raise IrcMsgException(
                caller,
                "No profile associated with your nick, please identify or " +
                "register first")
        if not newpass.isascii():
            raise IrcMsgException(
                caller, "Password may only contain ascii characters")
    ...
    def cmd_whoami(self, caller, cmd):
        ...
        if caller.level() is guestlevel:
            self.notice(caller, "You are %s, an unregistered guest" %
                        caller.nick)
        else:
    ...
    def cmd_whois(self, caller, cmd, nick):
        ...
        if nick and caller.is_confirmed():
            ...
            if nick and nick.lower() != caller.nick.lower():
                raise IrcMsgException(
                    caller, "You must have a confirmed account to view others")
            profile = caller.profile
            leveltxt = str(profile.level)
    ...
    def cmd_help(self, caller, cmd, command, verbose):
        ...
        try:
            if command:
                cmd, args = self.commands.parse_cmdline(
                    command, caller, forhelp=True)
    ...
    def cmd_channel_register(self, caller, cmd, chan):
        ...
        if chan.registered:
            raise IrcMsgException(
                caller, "Channel '%s' is already registered" % chan.name)
        chan.register()
    ...
    def cmd_channel_unregister(self, caller, cmd, chan):
        ...
        if not chan.registered:
            raise IrcMsgException(
                caller, "Channel '%s' is not registered" % chan.name)
        chan.unregister()
    ...
    def cmd_channel_roles(self, oper, cmd, chan):
        ...
        if chan.allow_guests and chan.default_policy_allow:
            ...
            self.notice(
                oper, "%s policy is set to allow but guests are denied" % chan.name)
        elif not chan.default_policy_allow:
            self.notice(oper, "%s policy is set to deny" % chan.name)
    ...
    def recv(self, caller, msg):
        print("XXXXX recv caller, msg = %r, %s" % (caller, msg))
        assert isinstance(msg, str)
        msg = msg.strip()
        try:
        ...
        print("XXXXX recv cmd, args = %r, %r" % (cmd, args))
        ...
        log.debug(
            ...
            % (
                caller.nick,
                cmd.funcname,
                ", ".join("%s=%s" % tup for tup in list(args.items())),))
