...
class Avatar(object):
    ...
    def cmd_identify(self, caller, cmd, nick, password):
        ...
        if not profile:
            raise IrcMsgException(
                caller, "No profile for %s, please register first" % nick)
    ...
    def cmd_register(self, caller, cmd, password):
        ...
        if self.core.channels.get_channel(password):
            raise IrcMsgException(
                caller, "Trying to register a channel? try the 'register channel' command")
    ...
    def cmd_unregister(self, caller, cmd, nick, password):
        ...
        if caller.level() is operlevel:
            ...
            if profile is caller.profile:
                raise IrcMsgException(
                    caller, "Server operators cannot unregister their own profile")
        else:
    ...
    def cmd_passwd(self, caller, cmd, oldpass, newpass):
        ...
        if not profile:
            raise IrcMsgException(
                caller, "No profile associated with your nick, please identify or register first")
